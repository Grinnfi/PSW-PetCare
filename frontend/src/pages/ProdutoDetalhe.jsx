...
export default function ProdutoDetalhe({ addToCart }) {
  ...
  const relacionados = useMemo(() => {
    ...
    return produtos
      .filter((p) => p.categoriaPet === produto.categoriaPet && p._id !== produto._id)
      .slice(0, 4)
  }, [produtos, produto])
    ...
    return (
      <div className="page" style={{ textAlign: 'center', padding: '80px 20px' }}>
        ...
        <p style={{ color: 'var(--muted)', marginBottom: 20 }}>
          Esse produto pode ter sido removido do catálogo.
        </p>
  ...
  const imagemProduto = pegarImagem(produto)
  ...
  return (
    <div id="page-produto-detalhe" className="page">
      <div className="breadcrumb">
        ...
        <span className="bc-sep">›</span>
        <span onClick={() => navigate(`/loja?categoria=${encodeURIComponent(produto.categoriaPet)}`)}>
          {produto.categoriaPet}
        </span>
        <span className="bc-sep">›</span>
      ...
      <div className="detalhe-layout">
        ...
        <div className="detalhe-info">
          <span className={`badge ${classeBadge(produto.cat)}`}>
            {produto.cat}
          </span>

          <h1 className="detalhe-nome">{produto.name}</h1>

          <p className="detalhe-unidade">
            Unidade: {produto.unit || 'unidade'}
          </p>
          ...
          <div className={`status-badge ${si.cls}`} style={{ marginBottom: 20, alignSelf: 'flex-start' }}>
            <span className="sdot" />
            {si.label}
            {!esgotado && ` (${produto.stock} disponíveis)`}
          </div>
          ...
          <button
            className="btn-add-cart-detalhe"
            disabled={esgotado}
            onClick={handleAdicionar}
          >
            {esgotado
              ? 'Produto Esgotado'
              : `Adicionar ao Carrinho — ${fmtPreco(produto.price * qtd)}`}
          </button>
        </div>
      </div>
      ...
      <div className="detalhe-tabs-card">
        <div className="detalhe-tabs">
          <button
            className={`detalhe-tab ${aba === 'descricao' ? 'active' : ''}`}
          ...
          <button
            className={`detalhe-tab ${aba === 'specs' ? 'active' : ''}`}
            onClick={() => setAba('specs')}
          >
            Especificações
          </button>
        </div>
        ...
        <div className="detalhe-tab-content">
          {aba === 'descricao' ? (
            ...
            <ul className="detalhe-specs-list">
              <li>
                <span>Categoria</span>
                <strong>{produto.cat}</strong>
              </li>
              <li>
                <span>Para</span>
                <strong>{produto.categoriaPet}</strong>
              </li>
              <li>
                <span>Unidade</span>
                <strong>{produto.unit || 'unidade'}</strong>
              </li>
              <li>
                <span>Estoque disponível</span>
                <strong>{produto.stock} {produto.unit || 'un.'}</strong>
              </li>
            </ul>
          )}
        </div>
      ...
      {relacionados.length > 0 && (
        <div className="home-section" style={{ padding: '32px 0 0' }}>
          <div className="section-title">
            Você também pode <span>gostar</span>
          </div>
